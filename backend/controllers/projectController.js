...
export const patchProject = async (req, res) => {
  try {
    ...
    if (data.startDate && data.endDate) {
      if (new Date(data.startDate) > new Date(data.endDate)) {
        return res.status(400).json({
          mensaje: "La fecha de inicio no puede ser mayor a la fecha de finalización.",
        });
      }
    }

    // Validar responsables si se actualiza ese campo
    if (data.responsibles) {
      const validResponsibles = await filtrarUsuariosExistentes(data.responsibles);
      if (validResponsibles.length !== data.responsibles.length) {
        return res
          .status(400)
          .json({ mensaje: "Alguno de los responsables no existe." });
      }
      data.responsibles = validResponsibles;
    }

    // Validar usuarios si se actualiza ese campo
    if (data.users) {
      const validUsers = await filtrarUsuariosExistentes(data.users);
      if (validUsers.length !== data.users.length) {
        return res
          .status(400)
          .json({ mensaje: "Alguno de los usuarios no existe." });
      }
      data.users = validUsers;
    }

    // Si se actualizan responsables o usuarios, fusionar ambos en un único arreglo
    if (data.responsibles || data.users) {
      const mergedUsers = [
        ...new Set([
          ...(data.responsibles || project.responsibles),
    ...
    // Actualizar el proyecto con los datos enviados (edición parcial)
    ...
    // Si se modificó el arreglo de usuarios, actualizar las referencias en el modelo User
    if (data.users) {
