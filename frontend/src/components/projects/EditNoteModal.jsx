...
import { deleteNote } from "@/lib/projects"; // ⬅️ al principio del archivo
...
export default function EditNoteModal({ isOpen, onClose, projectId, projectUsers, noteToEdit, onNoteEdited }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (onNoteEdited) {
        const fullUsers = projectUsers.filter(user => userWhoRecieves.includes(user._id));
        onNoteEdited(
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (onNoteEdited) {
        onNoteEdited(null, noteToEdit.index); // Le mandamos null para que NotesSection la elimine
      }
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="flex justify-between mt-6">
                ...
                <div className="flex justify-end gap-4">
                  ...
                  <button
                    type="submit"
                    className="px-4 py-2 bg-blue-600 transition duration-500 text-white rounded hover:bg-blue-700"
                    disabled={isLoading}
                  >
