                ...
                <div className="flex bg-primary-bg rounded-md border border-slate-200 p-0.5">
                    {["todas", "no completada", "completada"].map((filter) => (
                        <button
                            ...
                            className={`text-xs px-3 py-1 rounded flex items-center gap-1 transition-colors ${activeFilter === filter
                        ...
                        >
                            {filter === "no completada" && <span className="inline-block w-2 h-2 rounded-full bg-rose-500" />}
                            {filter === "completada" && <span className="inline-block w-2 h-2 rounded-full bg-emerald-500" />}
                ...
                {filteredNotes.length > 0 ? (
                                    ...
                                    <div className="p-4">
                                        ...
                                        <div className="flex items-center justify-between">
                                            ...
                                            <div className="flex items-center gap-2 flex-wrap">
                                                {assigned.map((user, i) => {
                                                    const name = user?.name || user?.toString() || "Usuario"
                                                    return (
                                                        <div key={i} className={`flex items-center gap-1 px-2 py-1 rounded-full text-xs bg-card text-primary-text ${getColorFromName(user.name)} border border-slate-300`} title={user.email || ""}>
                                                            <div className={`w-5 h-5 rounded-full flex items-center justify-center text-xs font-semibold ${getColorFromName(user.name)}`}>
                    ...
                    <div className="flex flex-col justify-center items-center h-full text-primary-text bg-primary-bg rounded-lg border border-slate-200 p-8">
                        ...
                        <p className="text-sm mt-2 text-secundary-text">
                            {activeFilter === "todas" ? 'Haz clic en "Añadir" para crear una nueva nota' : "Cambia el filtro o añade nuevas notas"}
                        </p>
                    </div>
                )}
            </div>
