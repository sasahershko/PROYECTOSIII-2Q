...
export default function PresupuestoDashboard({ data, onEditClick }) {
  ...
  const graficoPieData = [
    ...
    { name: 'Estudiantes', value: desglose.estudiantes?.reduce((sum, item) => sum + item.subtotal, 0) || 0 },
  ...
  const getColumnLabels = (tipo) => {
    switch (tipo) {
      ...
      case 'estudiantes':
        return columnasUnificadas.map(col => {
          if (col.key === 'col2') return { ...col, label: 'Número de Estudiantes' };
          if (col.key === 'col3') return { ...col, label: 'Horas Estimadas' };
          if (col.key === 'col4') return { ...col, label: 'Precio por Hora' };
          return col;
        });
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <div className="rounded-xl shadow-md border p-4">
          ...
          <ResponsiveContainer width="100%" height={270}>
            <PieChart>
              <Pie data={graficoPieData} dataKey="value" nameKey="name" outerRadius={100} label>
            ...
            <div className="flex justify-center mb-4">
              <ResponsiveContainer width="60%" height={280}>
                <LineChart data={seguimientoData}>
                  ...
                  <YAxis />
                  <Tooltip />
                  <Line type="monotone" dataKey="estimado" stroke="#60a5fa" strokeWidth={2} />
        ...
        <div className="rounded-xl shadow-md border p-4">
          {historialCambios.map((cambio, i) => (
            <div key={i} className="flex items-start gap-3">
              ...
              <div>
                <p className="font-semibold">{cambio.nombre} · <span className="text-sm text-gray-500">{cambio.fecha}</span></p>
